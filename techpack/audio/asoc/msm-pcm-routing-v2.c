		...
		if (!is_be_dai_extproc(i) &&
		   ...
		   (test_bit(fedai_id, &msm_bedais[i].fe_sessions[0]))) {
			...
			if (hifi_filter_enabled && (msm_bedais[i].sample_rate ==
				...
				bits_per_sample = msm_routing_get_bit_width(
							SNDRV_PCM_FORMAT_S32_LE);
#ifdef AUDIO_SPEAKER_3D
			if((i == 103) && (fe_dai_app_type_cfg[fedai_id][session_type][i].channel != 0)){
		...
		if (fdai->strm_id != INVALID_SESSION) {
			...
			if (hifi_filter_enabled && (bedai->sample_rate == 384000
				...
				bits_per_sample = msm_routing_get_bit_width(
							SNDRV_PCM_FORMAT_S32_LE);
#ifdef AUDIO_SPEAKER_3D
			if((be_id == 103) && (fe_dai_app_type_cfg[i][session_type][be_id].channel != 0)){
...
extern void msm_crus_pb_add_controls(struct snd_soc_component *component);
#endif
...
#ifdef CONFIG_MSM_CSPL
	msm_crus_pb_add_controls(&platform->component);
#endif
