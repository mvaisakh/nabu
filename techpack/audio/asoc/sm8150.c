...
#ifdef CONFIG_TARGET_PRODUCT_NABU
...
static int usbhs_direction_get(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	if (get_hw_version_platform() == HARDWARE_PLATFORM_NABU) {
...
#ifdef CONFIG_SND_SOC_TAS256X_FOR_DAVI
	btn_high[0] = 75;
	btn_high[1] = 225;
	btn_high[2] = 450;
	btn_high[3] = 500;
	...
	if (++mi2s_intf_conf[index].ref_cnt == 1) {
		...
		if (index == QUAT_MI2S) {
			ret_pinctrl = msm_set_pinctrl(pinctrl_info,
							STATE_MI2S_ACTIVE);
			if (ret_pinctrl) {
				pr_err("%s: MI2S TLMM pinctrl set failed %d, switching to gpio\n",
...
#ifndef CONFIG_TARGET_PRODUCT_NABU
	{
