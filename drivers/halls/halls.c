...
static int halls_configurate_irq(int in_GPIO, const char *ch_mark, void *arg,
				 int *out_irq)
{
	int iRet = 0, in_IRQ = 0;
...
static ssize_t halls_read(struct file *file, char __user *buf, size_t count,
			  loff_t *ppos)
{
	int iRead = 0;
	...
	if (g_in_flush == 1) {
		g_in_flush = 0;
		iRead = copy_to_user(buf, DEF_HALL_FLUSH,
				     strlen(DEF_HALL_FLUSH));
...
long halls_in_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case DEF_HALLS_IOCTRL_READ_HALL_1: {
		...
		iRet = copy_to_user(p_ReadValue, ch_data, strlen(ch_data));
		break;
	}
	...
	case DEF_HALLS_IOCTRL_STEUP: {
		if (g_in_opencounter == 0) {
			...
			iRet = halls_configurate_irq(DEF_HALL_GPIO_1, "hall-1",
						     (void *)g_st_halls,
						     &g_gpio_1_irq);
			if (iRet)
				pr_err("halls_configurate_irq failed!%d, iRet %d\n",
				       DEF_HALL_GPIO_1, iRet);
			g_in_firedirq = g_gpio_1_irq;

			iRet = halls_configurate_irq(DEF_HALL_GPIO_2, "hall-2",
						     (void *)g_st_halls,
						     &g_gpio_2_irq);
			if (iRet)
				pr_err("halls_configurate_irq failed! %d iRet %d\n",
			...
			HALLS_LOG(LOGTAG "g_gpio_2_irq %d, g_gpio_2_irq %d\n",
...
static int __init halls_init(void)
{
	...
	g_st_halls->hall_wakelock = wakeup_source_register(NULL, "dual_hall_ws");
